...
void find_k_bitedge_monte_carlo(std::vector<initial_vertex> & graph, uint nEdges, std::ifstream & stList)
{
	std::cout << std::endl;
		...
		for(int v = 0; v < numVertices; v++){
			...
			if( vdegree != 0 ){
				for( uint inbr = 0; inbr < vdegree; inbr++){
					...
					edgeBits.at(v).bits.push_back(temp_bits);
				}
			}
		}
...
int bitedge_traverse_run(
	...
	int k)
{
	std::stack<uint> worklist;
	std::stack<uint> exploring;
	uint v, w;
	int reached = 0;
	...
	if ( graph.at(v).nbrs.size() != 0) {
		for( uint i = 0; i < graph.at(v).nbrs.size(); i++){
			exploring.push(graph.at(v).nbrs.at(i).tgtIndex);
		}
	}else
	{
		return 0;
	...
	while (!worklist.empty())
	{
		...
		if ( vdegree != 0) {
			for( uint i = 0; i < vdegree; i++){
				...
				bool * ebits = edgeBits.at(v).bits.at(i);
				if(w == target){
					for(int i =0; i< k; i++){
						if( ebits[i] == true ){
							reached ++;
